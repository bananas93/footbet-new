...
export const getTournaments = createAsyncThunk('tournament/getTournaments', async (type?: TournamentType) => {
  ...
  return (data || []).map((item: any) => ({
    ...
    championsSlots: Number(item.champions_slots || item.direct_next_round || 0),
    europaSlots: Number(item.europa_slots || item.playoff_round || 0),
    relegationSlots: Number(item.relegation_slots || 0),
...
export const getOneTournament = createAsyncThunk('tournament/getOneTournament', async (id: number) => {
  ...
  return {
    ...
    championsSlots: Number(data.champions_slots || data.direct_next_round || 0),
    europaSlots: Number(data.europa_slots || data.playoff_round || 0),
    relegationSlots: Number(data.relegation_slots || 0),
